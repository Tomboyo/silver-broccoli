...
public class BoundedRetryBatchConsumer<K, V> implements Runnable {
  ...
  private final BatchConsumerCallback<K, V> worker;
  ...
  public static <K, V> BoundedRetryBatchConsumer<K, V> fromConfig(
      ...
      BatchConsumerCallback<K, V> worker) {
    ...
    try {
      ...
      while (true) {
        ...
        if (currentBatch.isPresent()) {
          ...
          if (batch.isDone()) {
            LOGGER.info("Committing batch: batch={}", batch);
  ...
  private static final class BoundedRetryWorker<K, V> implements BatchConsumerCallback<K, V> {
    private final int maxAttempts;
    private final Optional<String> dlt;
    private final BatchConsumerCallback<K, V> delegate;

    public BoundedRetryWorker(
        int maxAttempts, Optional<String> dlt, BatchConsumerCallback<K, V> delegate) {
