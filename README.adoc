...
This example silences all but com.github.tomboyo logging so we can more easily follow the program as it attempts to process messages. We can also inspect the `audit_log` database table to confirm that the program creates corresponding audit entries whose uniqueness is maintained by the idempotent inserts.
