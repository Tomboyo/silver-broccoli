...
We achieve bounded retry by tracking number of attempts before processing a message. If that number of attempts exceeds a threshold, we skip the message (that is, commit past it). The implementation may choose to submit the message to a DLT, log an error, or proceed silently depending on requirements.

Note that a dead-letter topic is a normal topic. When we dead-letter a message, we wrap it in a serializable type which conveys most of the original record fields unmodified and publish it to the designated dead-letter topic, like `mytopic.DLT`. We ensure there is enough context captured by the dead-letter record for engineers to research the root cause of errors later on.

In our example application, this is done completely in-memory. However, we could use a database table of kafka topic to track retries for each message. In order to protect against unrecoverable errors like OOME, the application should attempt to track the number of attempts, tll, or other metric _before_ processing the message. This pre-emptive strategy protects us against catastrophic errors like JVM OutOfMemeoryErrors. (If we tried to increment our number of attempts in a catch block for such an error, there's no guarantee we would ever successfully do so, and our retry would continue indefinitely.)
...
Dead-letter topics are the same as all other topics, so we simply start a consumer drawing from the dead-letter topic in order to access its contents.
