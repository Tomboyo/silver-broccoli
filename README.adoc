...
We achieve bounded retry by tracking number of attempts before processing a message. If that number of attempts exceeds a threshold, we skip the message (that is, commit past it). The implementation may choose to submit the message to a DLT, log an error, or proceed silently dependeing on requirements.

In our example application, this is done completely in-memory. However, we could use a database table to track retries for each message. In order to protect against unrecoverable errors like OOME, the application should attempt to increment the attempts column and make the decision to give up _before_ processing the message. That is, do not try to increment the attempts column as a way of handling errors like OOME. There is no guarantee the application will successfully access the database when the JVM has run out of memory. This approach could lead to infinite processing attempts if the task itself causes thse unrecoverable errors.
