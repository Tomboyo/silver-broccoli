...
[source,sh]
----
curl localhost:8081/actuator/bindings/producer-out-0/ -H 'content-type: application/json' -d '{"state": "PAUSED"}'
----
...
I have also asked link:https://stackoverflow.com/questions/68895758/spring-cloud-stream-with-kafka-binder-bindings-actuator-api-does-not-stop-prod[this question on stack overflow] to determine if pollable producers can be stopped and resumed.
